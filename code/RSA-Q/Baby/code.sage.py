

# This file was *autogenerated* from the file code.sage
from sage.all_cmdline import *   # import sage library

_sage_const_16 = Integer(16); _sage_const_1 = Integer(1)
from Crypto.Util.number import long_to_bytes

# 1. 输入题目给出的十六进制数据
# 注意：直接作为字符串传入，用 int(x, 16) 转换
sum_pq_hex = "0x1232fecb92adead91613e7d9ae5e36fe6bb765317d6ed38ad890b4073539a6231a6620584cea5730b5af83a3e80cf30141282c97be4400e33307573af6b25e2ea"
prod_plus1_hex = ""

e_hex = "0xe6b1bee47bd63f615c7d0a43c529d219"
d_hex = ""
enc_flag_hex = ""

# 2. 转换为整数
sum_pq = int(sum_pq_hex, _sage_const_16 )
prod_plus1 = int(prod_plus1_hex, _sage_const_16 )
d = int(d_hex, _sage_const_16 )
c = int(enc_flag_hex, _sage_const_16 )

# 3. 恢复模数 N
# N = (p+1)(q+1) - (p+q) - 1
N = prod_plus1 - sum_pq - _sage_const_1 
print(f"[*] Recovered N: {N}")

# 4. 直接解密
# m = c^d mod N
m = pow(c, d, N)

# 5. 转换为字符并打印
flag = long_to_bytes(m)
print(f"[+] Flag: {flag.decode()}")

